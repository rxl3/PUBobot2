...
@author_match
async def pick(ctx, match: bot.Match, player: Member, role: str | None = None):
	if role:
		await match.draft.pick(ctx, ctx.author, player, role=Role[role])
	else:
		await match.draft.pick(ctx, ctx.author, player)
